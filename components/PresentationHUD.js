...
export default function PresentationHUD({ sedId, sedName, llaveName, sedsList = [], localDatabase = {}, circuitEntries = [], circuitStatus = 'cargado', onSelectSed, onSelectLlave, onSelectCircuit, currentMapStyle, onPrevSed, onNextSed, onToggleMapStyle, onEnterEditMode, onExportExcel }) {
  ...
  return <div className="presentation-hud">
    ...
    <div style={{ display: 'flex', gap: '8px' }}><button className="hud-btn" onClick={onPrevSed}><i className="fa-solid fa-chevron-left"></i> SED Ant.</button><button className="hud-btn" onClick={onNextSed}>SED Sig. <i className="fa-solid fa-chevron-right"></i></button>{onExportExcel && <button className="hud-btn" onClick={onExportExcel} style={{ background: 'rgba(46, 125, 50, 0.25)', borderColor: '#2e7d32', color: '#4caf50' }}><i className="fa-solid fa-file-excel"></i> Excel por SED</button>}<button className="hud-btn" onClick={onToggleMapStyle}><i className={`fa-solid ${currentMapStyle === 'clean' ? 'fa-layer-group' : 'fa-map-location-dot'}`}></i><span>{currentMapStyle === 'clean' ? 'Mapa Limpio' : 'Mapa Detallado'}</span></button><button className="hud-btn" onClick={onEnterEditMode} style={{ background: 'rgba(255, 171, 0, 0.15)', borderColor: '#ffab00', color: '#ffab00' }}><i className="fa-solid fa-pen-to-square"></i> Modo Edición</button></div>
  </div>;
}
